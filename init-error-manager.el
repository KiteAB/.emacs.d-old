...
(defvar kiteab/all-the-config-errors nil
...
(defconst kiteab/error-from
	'("error from [[file:" "][" "]]")
	"The error-from strings")

;;;###autoload
(cl-defun kiteab/error-check (origin &rest options)
				...
				(:file-exists
				 (let
					 ...
					 (if (not (file-exists-p file-path))
							 (progn
								 (setf kiteab/all-the-config-errors
											 (append kiteab/all-the-config-errors
															 (list
																(cl-concatenate 'string "File does not exist: '" file-path "', "
																						 (car kiteab/error-from) (car origin) (cadr kiteab/error-from)
																						 (cadr origin) (caddr kiteab/error-from)))))
								 (message "The %s file is not exists." file-path))
						 (cl-return-from kiteab/error-check t)))))))
	nil)

;;;###autoload
(defun kiteab/error-close ()
...
;;;###autoload
(defun kiteab/error-show ()
	"Show all the errors in the while of starting emacs."
	(interactive)
	(if (null kiteab/all-the-config-errors)
			(message "There is no error while starting emacs.")
		(split-window-below)
		(switch-to-buffer "*All-Errors*")
		(org-mode)
		(dolist (err kiteab/all-the-config-errors)
			(insert err)
			(insert "\n"))
		(local-set-key (kbd "q") 'kiteab/error-close)
